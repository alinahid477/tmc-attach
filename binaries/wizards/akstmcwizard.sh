...
function doAKSTMCWizard () {
    ...
    if [[ ! -z $tmcattachurl ]]
    then
        source $HOME/binaries/wizards/akscreateandattach.sh -g $resourcegroup -n $clustername  -c $nodecount -s $nodevmsize -d $nodeosdisksize --tmc-attach-url $tmcattachurl
    else
        source $HOME/binaries/wizards/akscreateandattach.sh -g $resourcegroup -n $clustername  -c $nodecount -s $nodevmsize -d $nodeosdisksize --tmc-cluster-group $tmcclustergroup
    fi
